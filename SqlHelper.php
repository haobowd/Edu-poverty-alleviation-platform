...
class SqlHelper
{
    ...
    public function execute_dql($sql)
    {
        ...
        $res = mysqli_query($sql, $this->conn) or die(mysqli_error($this->conn));
        return $res;
    ...
    public function exectue_dql_fenye($sql1, $sql2, $fenyePage)
    {
        ...
        //这里我们查询了要分页显示的数据
        $res = mysqli_query($sql1, $this->conn) or die(mysqli_error($this->conn));
        //$res=>array()
        $arr = array();
        //把$res转移到$arr
        while ($row = mysqli_fetch_assoc($res)) {
        ...
        $res2 = mysqli_query($sql2, $this->conn) or die(mysqli_error($this->conn));
